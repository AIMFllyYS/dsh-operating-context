	...
	factory: (require) => {
		...
		*/
		...
		function Loaded({ controller, useSnapshot, t }) {
			...
			const target = draft.custom ? typed !== void 0 && Number.isInteger(typed) && typed > 0 ? typed : void 0 : draft.preset ?? state.current;
		...
		var OperatingContextStore = class {
			...
			*/
			async apply(target) {
				...
				try {
					if (!Number.isInteger(target) || target <= 0) throw new CodedError("window must be a positive integer", WRITE_BLOCKED.invalidWindow);
					...
					const namespaces = new Map(document.namespaces.map((view) => [view.ns, view]));
					for (const { ns, ops, revision } of this.groupOps(namespaces, target)) unwrap(await this.api.settings.mutate({
						ns,
						ops,
